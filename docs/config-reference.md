...
|-------|------|---------|-------------|
| `lang` | `python` \| `node` \| `go` \| `java` \| `rust` \| `ruby` \| `php` \| `dotnet` | — | **Required.** Language runtime. |
...
| `command` | list[string] | `null` | Start command (exec form), e.g. `["uvicorn", "main:app", ...]`. Auto-detected at `init`; edit it if the smoke test fails. When unset, the Dockerfile falls back to a per-framework heuristic. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `provider` | `github` \| `gitlab` \| `bitbucket` | `github` | CI platform to generate for. (Only `github`/`gitlab` currently generate output; other detected providers are clamped to `github` at `init`.) |
